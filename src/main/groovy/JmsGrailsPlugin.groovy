...
class JmsGrailsPlugin extends Plugin {
	...
	static LOG = LogFactory.getLog('com.netvitesse.nvconnect.JmsGrailsPlugin')
	...
	def author = "Jean Silga"
	def authorEmail = "jsilga@martellotech.com"
	...
	Closure doWithSpring() { {->
			jmsConfig = grailsApplication.config.jms
	...
	void onChange(Map<String, Object> events) {
		...
		events.each{ pl, event->
			if (event.source && event.ctx) {
				...
				} else if (grailsApplication.isServiceClass(event.source)) {
	...
	void onConfigChange(Map<String, Object> events) {
		...
		events.each{ pl, event ->
			...
			def newJmsConfig = grailsApplication.config.jms
			newJmsConfig.putAll(event.source.jms)
			...
			if (newJmsConfigHash != jmsConfigHash) {
				...
				def jmsServiceClass = grailsApplication.classLoader.reloadClass(grailsApplication.mainContext.jmsService.class.name)
